...
import { logoutUserAction, updatePasswordAction } from "../../../redux/slices/Users/usersSlices";
...
export default function UpdatePassword() {
  ...
  const usersPass = useSelector(state => state?.users)
  const {passwordUpdated,loading, appErr, serverErr, isPasswordUpdated, userAuth} = usersPass;
  ...
  const formik = useFormik({
    ...
    onSubmit : values =>  {
      dispatch(updatePasswordAction({password: values.password}));
    },
    validationSchema: formSchema
  });

  //redirect
  if (isPasswordUpdated) return <Redirect to={dispatch(logoutUserAction())} />;
  return (
